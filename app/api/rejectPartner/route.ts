  ...
  if (req.method === "POST") {
    try {
      ...
      if (currentUser._id.equals(targetUser._id)) {
        return NextResponse.json(
          { message: "Cannot send or reject partner request to yourself" },
          { status: 400 }
        );
      }
      ...
      if (!!requesterFromPartnersList) {
        ...
        const targetUserUpdate = await User.findOneAndUpdate(
          { _id: request?.userId },
          {
            $push: {
              notifications: {
                type: NotificationTypeE.partnerReject, // "partnerExcept",
                content: `${currentUser.name} ${currentUser.surname} rejected your partner request`,
                user: session.user._id,
              },
            },
            $pull: {
              partners: { user: session.user._id },
            },
            // $set: {
            //   "partners.$[elem].status": PartnerStatusE.rejected,
            // },
          }
          //   {
        ...
        return NextResponse.json(
          {
            message: `Partnership requester was informed of rejection successfully`,
          },
          { status: 200 }
        );
      } else {
